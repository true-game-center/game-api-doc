...
## 订单串联标识（gameSessionId / roundId / transactionId / tickId）

本节在现有 `user.score.change` 模型上补充订单串联字段，用于把同一游戏会话、同一回合内的下注、派彩、冲正流水关联起来，方便后续对账。

标识层级（从大到小）：

```text
gameSessionId  ⊃  roundId  ⊃  transactionId  ⊃  tickId
游戏会话           回合        钱包交易/幂等键      子事件
```

| Field | Required | Description |
|-------|----------|-------------|
| `gameSessionId` | No | 游戏侧生成的游戏会话 ID。Game Center 不生成该字段、不维护会话表，只在 `user.score.change` 收到后落到 `game_discount_record.game_session_id`。 |
| `roundId` | No | 回合 ID，用于串联同一回合的下注与派彩。强烈建议同一回合的 `debit` 与后续 `credit` / `return` 传同一个 `roundId`。 |
| `transactionId` | Yes | 单次账务动作的交易 ID / 幂等键。每次 `debit`、`credit` 通常各有自己的 `transactionId`；`return` 复用其所冲正原 `debit` 的同一个 `transactionId`。 |
| `tickId` | No | 回合内子事件 ID，仅 tick 型游戏需要，例如捕鱼每一炮、连续游戏的每一步。 |

`game_discount_record.id` 是 Game Center 本地流水主键，并作为 `bizId` 传给下游 `/oapi/score/cost`；钱包侧流水可通过 `card_score_flow.biz_id = game_discount_record.id` 反查这些串联字段。以上新增字段全部可选，旧接入方不传时保持兼容。
...
```json
{
  ...
  "params": {
    ...
    "groupId": "VOICE_ROOM_1000",  // Voice room ID, optional
    "gameSessionId": "GS-20260629-001",  // Game session ID generated by the game side, optional
    "roundId": "R-77f0a1",  // Round ID used to link debit and credit records, optional but recommended
    "tickId": "T-001",  // Tick/sub-event ID, optional and only needed for tick-based games
    "score": 999.00,  // Score/points
    "cost": 10.00,  // Bet amount, optional
    "win": 999.00,  // Payout amount, optional
    ...
    "ticketId": 123456789,  // Coupon ID, optional
...
|-------|------|----------|-------------|
| `a0` | Long | Yes | Root agent / KOL user ID. |
| `userId` | Long | Yes | User ID. |
| `category` | String | Yes | Game category ID. |
| `providerId` | String | Yes | Game provider ID. |
| `gameId` | String | No | Game ID. |
| `groupId` | String | No | Voice room ID. |
| `gameSessionId` | String | No | Game session ID generated by the game side. Game Center stores it but does not generate it. |
| `roundId` | String | No | Round ID used to link related debit, credit, and return records. |
| `tickId` | String | No | Tick/sub-event ID for tick-based games. |
| `score` | BigDecimal | Yes | Score/points changed by this operation. |
| `cost` | BigDecimal | No | Bet amount. For debit records, Game Center falls back to `score` when `cost` is omitted. |
| `win` | BigDecimal | No | Payout amount. For credit records, Game Center falls back to `score` when `win` is omitted. |
| `transactionId` | String | Yes | Transaction ID and idempotency key for this score-change operation. |
| `opType` | String | Yes | Operation type: `debit`, `credit`, `return`, or `gift`. |
| `currency` | String | Yes | Currency: `GOLD`, `VND`, `USD`, `FAM`, etc. |
| `ticketId` | Long | No | Coupon ID. |
| `dataset` | String | No | Passthrough dataset to the A0 score service. |
| `enableJackpot` | Integer | No | Enable JackPot: `0` = No, `1` = Yes. |
| `ext` | String | No | Passthrough string to the A0 score service. |
...
`gameSessionId`, `roundId`, and `tickId` are optional order-linking fields. Game Center receives them from the game side/proxy and stores them in `game_discount_record`; Game Center does not generate `gameSessionId`. Use the same `roundId` for a debit record and its related credit/return records so bet and settle can be reconciled. `transactionId` remains the idempotency key for a single score-change operation; `return` uses the same `transactionId` as the original debit record it reverses.
